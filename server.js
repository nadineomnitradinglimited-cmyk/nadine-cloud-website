...
const server = http.createServer((req, res) => {
  ...
  if (req.method === 'POST' && urlPath === '/api/admin/cleanup-migration-test-users') {
    // temporary one-off route: deletes the throwaway test accounts created
    // while verifying Phase 2 (auth) cross-service compatibility. Remove
    // once used.
    require('./db').getPool()
      .query("DELETE FROM users WHERE email LIKE '%migration-test-%@example.com' OR email LIKE '%-created-test-%@example.com' RETURNING email")
      .then((result) => {
